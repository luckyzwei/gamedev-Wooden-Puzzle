...
{
    int levelNumber = 1;
    string[] toolbarStrings = new string[] { "Editor", "Helps", };
    ...
    private void OnFocus()
    {
        ...
        if (EditorSceneManager.GetActiveScene().name == "game")
        {
            ...
            var clm = Camera.main.GetComponent<ContainAssistant>();
            blockTextures = new Texture[clm.cItems.Count];
            blockContents = new GUIContent[clm.cItems.Count];
            for (int i = 0; i < blockTextures.Length; i++)
            {
                blockTextures[i] = clm.cItems[i].item.GetComponent<SpriteRenderer>().sprite.texture;
            }
            for (int i = 0; i < blockTextures.Length; i++)
            {
